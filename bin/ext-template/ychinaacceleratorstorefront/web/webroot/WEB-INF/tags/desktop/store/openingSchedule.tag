...
<c:if test="${not empty openingSchedule}">
	<ycommerce:testId code="storeDetails_table_openingSchedule_label">
		<table class="store-openings weekday_openings">${jsCR}
			<tbody>${jsCR}
				<c:forEach items="${openingSchedule.weekDayOpeningList}" var="weekDay">
					<tr class="${weekDay.closed ? 'weekday_openings_closed' : 'weekday_openings'}">${jsCR}
						<td class="weekday_openings_day">${weekDay.weekDay}</td>${jsCR}
						<td class="weekday_openings_times">${jsCR}
							<c:choose>
								<c:when test="${weekDay.closed}" >
										<spring:theme code="storeDetails.table.opening.closed" />${jsCR}
								</c:when>
								<c:otherwise>
									${weekDay.openingTime.formattedHour} - ${weekDay.closingTime.formattedHour}${jsCR}
								</c:otherwise>
							</c:choose>
						</td>${jsCR}
					</tr>${jsCR}
