		...
		<a href="${productUrl}" title="${product.name}" class="productMainLink">
			<div class="thumb">
				...
				<c:if test="${not empty product.potentialPromotions and not empty product.potentialPromotions[0].productBanner}">
					...
					<c:set value="${ycommerce:lazyLoadOrStandardImageTag('promo lazy',product.potentialPromotions[0].productBanner.url,product.potentialPromotions[0].description,commonResourcePath.concat('/images/grey.gif'))}" var="img"/>
					${img}
				</c:if>
			</div>
